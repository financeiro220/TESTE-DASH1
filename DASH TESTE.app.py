...
# Customização CSS para emular perfeitamente a sua referência
st.markdown("""
    <style>
    ...
    .kpi-card { background-color: #111827; border: 1px solid #1F2937; border-radius: 8px; padding: 15px; position: relative; }
...
# 3. FILTROS (Na barra lateral para manter o visual limpo)
...
with col_g1:
    ...
    # Organiza meses cronologicamente para o gráfico de linha
    df_linha = df.copy()
...
with col_g2:
    ...
    # Cores condicionais (Verde se dentro da meta de 30%, Vermelho se acima)
...
# Agrupando por unidade para descobrir os campeões do período selecionado
...
with col_r1:
    ...
    st.markdown(f'<div class="ranking-card"><div class="ranking-title">🥇 Maior Faturamento</div><div class="ranking-value">{top_fat}</div></div>', unsafe_allowed_html=True)
with col_r2:
    top_res = df_rank.sort_values(by="Resultado", ascending=False).iloc[0]["Unidade"]
    st.markdown(f'<div class="ranking-card"><div class="ranking-title">💰 Maior Resultado</div><div class="ranking-value">{top_res}</div></div>', unsafe_allowed_html=True)
with col_r3:
    top_tkt = df_rank.sort_values(by="Ticket", ascending=False).iloc[0]["Unidade"]
    st.markdown(f'<div class="ranking-card"><div class="ranking-title">🎯 Maior Ticket Médio</div><div class="ranking-value">{top_tkt}</div></div>', unsafe_allowed_html=True)
with col_r4:
    top_flx = df_rank.sort_values(by="Clientes", ascending=False).iloc[0]["Unidade"]
    st.markdown(f'<div class="ranking-card"><div class="ranking-title">👥 Maior Fluxo (Cli)</div><div class="ranking-value">{top_flx}</div></div>', unsafe_allowed_html=True)
with col_r5:
    top_cmv = df_rank.sort_values(by="CMV%").iloc[0]["Unidade"]
    st.markdown(f'<div class="ranking-card"><div class="ranking-title">✅ CMV Mais Eficiente</div><div class="ranking-value">{top_cmv}</div></div>', unsafe_allowed_html=True)
with col_r6:
    top_cmo = df_rank.sort_values(by="CMO%").iloc[0]["Unidade"]
    st.markdown(f'<div class="ranking-card"><div class="ranking-title">⚡ CMO Mais Eficiente</div><div class="ranking-value">{top_cmo}</div></div>', unsafe_allowed_html=True)
...
# Formatação final para exibição na tela
st.dataframe(
